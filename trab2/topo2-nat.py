...
class BasicTopo(Topo):
    def build(self, **_opts):
        ...
        server1 = self.addHost('server1', ip='8.8.8.8/16',
                               defaultRoute='via 8.8.254.254')
        server2 = self.addHost('server2', ip='8.8.4.4/16',
                               defaultRoute='via 8.8.254.254')

        self.addLink(host1, sw1)
        self.addLink(host2, sw1)
        self.addLink(sw1, router, intfName2='r-eth0',
                     params2={'ip': '10.1.1.254/24'})

        self.addLink(server1, sw2)
        self.addLink(server2, sw2)
        self.addLink(sw2, router, intfName2='r-eth1',
                     params2={'ip': '8.8.254.254/16'})
